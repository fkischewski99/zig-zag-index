// =============================================================================
// Zigzag_Index_Auswahl.ijm
// Zickzack-Index von ZO-1-Zell-Zell-Kontakten — mit Serienauswahl
//
// Wie Zigzag_Index.ijm, aber mit Dialog zur Auswahl einzelner Serien
// aus der LIF-Datei.
//
// Voraussetzungen:
...
// ---- Serienauswahl-Dialog ----
Dialog.create("Serien auswaehlen");
Dialog.addMessage("Serien zum Verarbeiten auswaehlen:");
for (s = 0; s < seriesCount; s++) {
    Dialog.addCheckbox(seriesNames[s], true);
}
Dialog.show();

selectedSeries = newArray(seriesCount);
nSelected = 0;
for (s = 0; s < seriesCount; s++) {
    selectedSeries[s] = Dialog.getCheckbox();
    if (selectedSeries[s]) nSelected++;
}

if (nSelected == 0) exit("Keine Serien ausgewaehlt.");
print("[INFO] " + nSelected + " von " + seriesCount + " Serien ausgewaehlt.");
...
// ---- Hauptschleife: Ausgewaehlte Serien durchgehen ----
for (s = 0; s < seriesCount; s++) {
    if (!selectedSeries[s]) continue;
    processedCount++;
    ...
    print("================================================");
    print("[INFO] Serie " + processedCount + "/" + nSelected + ": " + seriesName);
    ...
    waitForUser("Serie: " + seriesName + " (" + processedCount + "/" + nSelected + ") — Kontakte markieren",
